    ...
    <div className="grid gap-5">
      <div className="h-[280px] w-full">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={taskCategoryData} margin={{ top: 20, right: 12, left: 0, bottom: 0 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="rgba(148,163,184,0.24)" vertical={false} />
            <XAxis dataKey="category" axisLine={false} tickLine={false} tick={axisTickStyle} tickMargin={12} />
            <YAxis axisLine={false} tickLine={false} tick={axisTickStyle} tickMargin={12} width={32} />
            <Tooltip
      ...
      <div className="grid grid-cols-2 gap-3">
        {taskCategoryData.map((entry) => (
          <div key={entry.category} className="flex items-center gap-2 text-sm text-slate-900 dark:text-slate-100">
            <span className="inline-block h-3 w-3 rounded-full shadow-inner" style={{ backgroundColor: entry.color }} />
